                ...
                $.post(URL, formData, function(data, textStatus, jqXHR) {
                    ...
                    if (data.success) {
                        ...
                        if(mess.readerValidationPass === false) {
                            err = ['Try running the input through the <b><a href="#validator-page">Validator</a></b>.'];
    ...
    $('body').one('pagecontainershow', function(event, ui) {
        JSV.init({
            ...
            if (window.jsvInitPath) {
                JSV.expandNodePath(window.jsvInitPath.split('-'));
            } else {
                JSV.resetViewer();
                //initialize welcome popup
                $( '#popup-welcome' ).enhanceWithin().popup().popup('open');
            }
        });
    });
