(function($) {
    ...
    var initTranslator = function() {
        var opts = {
            ...
            on: {
                load: function(e, file) {
                    ...
                    try {
                      if (file.type === 'application/json' || $('#select-reader').val().indexOf('Json') > -1) {
                        JSON.parse(data);
        ...
        //setup example links
        $('.load-fgdc').each(function(idx, link) {
            var ljq = $(link);
            ljq.on('click', function(evt) {
                evt.preventDefault();

                $.get(link.href, null, null, 'text').done(function(fetched) {
                    $('#' + ljq.data('target')).val(fetched);
                }).fail(function(jqXHR, textStatus, errorThrown) {
                    JSV.showError('Failed to load FGDC example: ' + errorThrown);
                });
            });
        });
        ...
        JSV.createXMLPre = function(el, obj, title, exp, raw) {
            ...
            var saveBtn = $('<a href="#" class="ui-btn ui-mini ui-icon-arrow-d ui-btn-icon-right">Save to File</a>').click(function() {
              var title = title|| $('#select-writer-button').text().replace(' ','-') + '-' + Date.now();
              var result = obj.data;
              var writerType = $('#select-writer').val().indexOf('Json') > -1 ? 'json' : 'xml';

              window.saveAs(
                new Blob([result], {
                  type: 'application/'+ writerType +';charset=utf-8'
                }),
                title + '.' + writerType
              );
            });
        ...
        */
        ...
        $('#button-translate').click(function() {
            ...
            try {
              if ($('#select-reader').val().indexOf('Json') > -1) {
                data = JSON.parse($('#textarea-translator').val());
              } else {
                data = $('#textarea-translator').val();
              }
            } catch(e) {
                JSV.showError('Unable to parse JSON: <br/>' + e);
            }
                    ...
                    if (data.success) {
                        if(data.data) {
                          if(/^{.+\}$/.test(data.data)){
                            ...
                            raw = (data.data.match(/^<.*!DOCTYPE.*(html|HTML)\/?>/) !== null);
                          }
